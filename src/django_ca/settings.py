...
SECRET_KEY = project_setting("SECRET_KEY", sections=["project", "security"])

signup_token_expiry = project_setting(
    "SIGNUP_TOKEN_EXPIRY", sections=["project", "tokens"], rtype=dict
)
SIGNUP_TOKEN_EXPIRY = timedelta(**signup_token_expiry)
access_token_expiry = project_setting(
    "ACCESS_TOKEN_EXPIRY", sections=["project", "tokens"], rtype=dict
)
ACCESS_TOKEN_EXPIRY = timedelta(**access_token_expiry)
refresh_token_expiry = project_setting(
    "REFRESH_TOKEN_EXPIRY", sections=["project", "tokens"], rtype=dict
)
REFRESH_TOKEN_EXPIRY = timedelta(**refresh_token_expiry)
# endregion

# region Application definition
DEBUG = project_setting("DEBUG", sections=["project", "app"], rtype=bool)
BASE_API_SCHEME = project_setting("BASE_API_SCHEME", sections=["project", "servers"])
BASE_API_DOMAIN = project_setting("BASE_API_DOMAIN", sections=["project", "servers"])
BASE_API_PORT = project_setting(
    "BASE_API_PORT", sections=["project", "servers"], rtype=int
)
BASE_APP_SCHEME = project_setting("BASE_APP_SCHEME", sections=["project", "servers"])
BASE_APP_DOMAIN = project_setting("BASE_APP_DOMAIN", sections=["project", "servers"])
BASE_APP_PORT = project_setting(
    "BASE_APP_PORT", sections=["project", "servers"], rtype=int
)
...
NO_REPLY_USER_PART = project_setting(
    "NO_REPLY_USER_PART", sections=["project", "app", "email"]
)
NO_REPLY_EMAIL_PART = project_setting(
    "NO_REPLY_EMAIL_PART", sections=["project", "app", "email"]
)
NO_REPLY_EMAIL = f'"{NO_REPLY_USER_PART}" <{NO_REPLY_EMAIL_PART}>'
EMAIL_BACKEND = project_setting("EMAIL_BACKEND", sections=["project", "app", "email"])
email_file_path = project_setting(
    "EMAIL_FILE_PATH", sections=["project", "app", "email"]
)
EMAIL_FILE_PATH = BASE_DIR.joinpath(email_file_path)
email_template_dir = project_setting(
    "EMAIL_TEMPLATE_DIR", sections=["project", "app", "email"]
)
...
OPTIMUS_PRIME = project_setting(
    "OPTIMUS_PRIME", sections=["project", "app", "optimus"], rtype=int
)
OPTIMUS_INVERSE = project_setting(
    "OPTIMUS_INVERSE", sections=["project", "app", "optimus"], rtype=int
)
OPTIMUS_RANDOM = project_setting(
    "OPTIMUS_RANDOM", sections=["project", "app", "optimus"], rtype=int
)
...
RSA_KEY_SIZE = project_setting(
    "RSA_KEY_SIZE", sections=["project", "certificates"], rtype=int, default=4096
)
...
# region Databases
db_name = project_setting("DB_NAME", sections=["project", "database"])
DATABASES = {
